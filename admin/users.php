...
// Get Records
$sql = "SELECT id, full_name, email, phone, role, status, created_at FROM users $whereSql ORDER BY created_at DESC LIMIT $limit OFFSET $offset";
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="px-6 py-5 border-b border-gray-100 bg-gray-50/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <h3 class="text-lg font-bold text-gray-900">Customer Management</h3>
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-white border-b border-gray-200 text-xs uppercase tracking-wider text-gray-500">
                    ...
                    <th class="px-6 py-4 font-semibold text-right">Account Status</th>
                </tr>
            </thead>
                ...
                <?php else: ?>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-600">
                            <?= date('M j, Y H:i', strtotime($u['created_at'])) ?>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex flex-col items-end gap-2">
                                <a href="view-user.php?id=<?= $u['id'] ?>" class="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-semibold rounded-lg transition-colors inline-flex items-center gap-1">
                                    <span class="material-symbols-outlined text-[14px]">visibility</span> View Profile
                                </a>
